...
function promptToSave() {
    if (confirm('გნებავთ ატვირთოთ ჩანაწერები ?\nთუ შეცდომა გაქვთ დაშვებული,' +
            ' აირჩიეთ cancel წაშალეთ არასაურველი ჩანაწერი და მერე ატვირთეთ.')) {
        saveRecordings();
    }
...
function uploadNextClip() {
    document.querySelector('.progress-display').innerText = 'ატვირთვა მიმდინარეობს ' +
    ...
    xhr.onload = function (e) {
        if (this.status === 200) {
            ...
            var uploadUrl = '/upload?word=' + toAscii(word) + '&_csrf_token=' + csrf_token;
            ...
            ajaxRequest.onreadystatechange = function () {
                if (ajaxRequest.readyState === 4) {
                    if (ajaxRequest.status === 200) {
                        ...
                        alert('უი! რაღაც გაიმაზა: ' + ajaxRequest.status);
                    }
                }
            };
